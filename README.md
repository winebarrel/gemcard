...
- `https://gemcard.winebarrel.workers.dev/gems/<name>` にアクセスすると、RubyGems の JSON API から gem 名 / version / 概要 / DL 数 / authors を取得し、`og:*` と `twitter:card` メタタグを埋めた HTML を返す。これを X / Slack に貼るとカード表示される。
- `og:image` は `/img/<name>.png` で動的生成。gem 名・version・概要・authors・DL 数を並べた 1200x630 PNG を [workers-og](https://github.com/kvnang/workers-og) (Satori + resvg) で生成する。
